exports.process = async ({ log, record, db, dynamo }) => {
  ...
  // If no customer id, throw
  if (!customerId) throw Error('Undefined customer id passed in')

  log.log({ amount, timestamp, customerId, description })
  // Subtract 3% for stripe percentage fee and 3% for our fee
  let donationAmount = amount * 0.94
  ...
  const userId = (await db.findUserId({ customerId })).toString()
  ...
  log.log({ success: true, customerId, donationAmount, description })
  return { success: true }
}
